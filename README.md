...
`sin` だけの波は規則的に見える。Simplex Noise を加えることで有機的なゆらぎになる。

GLSLには標準のノイズ関数がないため、Simplex Noise のアルゴリズムを自前実装して `fragment.glsl` に組み込む。
```glsl
// 使い方
float noise = snoise(uv * 3.0 + uTime * 0.2) * 0.5 + 0.5;
```

- `uv * 3.0` ：スケール（大きいほど細かいノイズ）
- `+ uTime * 0.2` ：時間を加えることでノイズが動く
- `* 0.5 + 0.5` ：出力を `-1〜1` から `0〜1` に正規化
...
`pow()` で波の値を累乗すると暗い部分がより暗く、明るい部分との差が広がる。値が大きいほどコントラストが強くなる。

**滑らかな色の切り替え**
```glsl
color = mix(color, colorC, smoothstep(0.4, 0.9, wave));
```

`smoothstep(a, b, x)` は `x` が `a` 以下で0、`b` 以上で1、間は滑らかに補間される。`mix()` と組み合わせることで特定の波の強さのときだけ色が鮮やかに浮き上がる。